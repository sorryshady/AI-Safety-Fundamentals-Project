...
class Qlearning:
    ...
    def adjust_reward(self, reward, done, state, map_size):
        """
        ...
        """
        high_reward_states = [
            (map_size - 1, 0), (0, map_size - 1)]  # Example of non-goal states
        row = state // env.unwrapped.ncol
        col = state % env.unwrapped.ncol

        # Check if the current state is in the high reward states
        if (row, col) in high_reward_states:
            return 1  # High reward for specific non-goal tiles

        # Original reward modifications for goal and hole
        if done and reward == 1:
            return 5  # Reward for reaching the goal
        elif done and reward == 0:
            return -1  # Punishment for falling into a hole
        else:
            return -0.05  # Punishment for taking a step
    ...
    for run in range(params.n_runs):
        ...
        for episode in tqdm(
            ...
            while not done:
                ...
                adjusted_reward = learner.adjust_reward(
                    reward, done, state, map_size)
